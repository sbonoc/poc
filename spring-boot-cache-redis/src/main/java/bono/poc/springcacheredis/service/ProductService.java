...
public class ProductService {
    ...
    public Optional<ProductModel> getProductWithoutCache(String id) {
        return getProductModel("getProductWithoutCache", id);
    }

    @Timed
    @Counted
    @Cacheable(cacheManager = "redissonCacheManager")
    public Optional<ProductModel> getProductWithRedissonCache(String id) {
        return getProductModel("getProductWithRedissonCache", id);
    }

    @Timed
    @Counted
    @Cacheable(cacheManager = "caffeineCacheManager")
    public Optional<ProductModel> getProductWithCaffeineCache(String id) {
        return getProductModel("getProductWithCaffeineCache", id);
    }

    private Optional<ProductModel> getProductModel(String methodName, String id) {
        log.debug("[{}] Getting product with id {} from Redis (the central cache)", methodName, id);
